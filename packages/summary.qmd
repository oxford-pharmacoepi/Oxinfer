...
get_cran_release_history <- function(package_name) {
  ...
  url_exists <- function(url) {
    con <- curl::curl(url)
    on.exit(close(con))
    res <- try(open(con, "rb"), silent = TRUE)
  ...
  for (i in seq_along(url)) {
    ...
    if (!url_exists(url[i])) {
      cli::cli_inform(c("!" = "{package_name[i]} is not in CRAN"))
      releases[[i]] <- NULL
    } else {
...
plot_dependencies <- function(suggests, remove = NULL){
edge_list <- list()

if(!is.null(remove)){
pkgs <- pkgs[! pkgs %in% remove]  
}
...
### Imports 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
plot_dependencies(suggests = FALSE, remove = c("EpiStandard"))
```
