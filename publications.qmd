...
-   <span class="author_publication">Burn E, Català M</span>. *Tidy R programming with the OMOP Common Data Model* (first edition). Zenodo. <https://doi.org/10.5281/zenodo.17532124>. 
...
authors <- c(
  ...
  "lopez guell, kim", "alcalde herraiz, marta", "mercade besora, nuria",
  "campanile, cecilia", "rowlands, elin"
)

numberTries <- 100
i <- 0
x <- NULL
while (i < numberTries & is.null(x)) {
  x <- tryCatch({
    pubmedR::pmApiRequest(
      query = paste0(authors, "[Author]", collapse = " OR "), 
      limit = Inf
    )
  },
...
text <- data |>
  ...
  dplyr::mutate(
    sum = dplyr::select(data, dplyr::all_of(ours$last_name)) |> 
      rowSums(na.rm = TRUE),
    group = dplyr::if_else(
      .data$year < 2022, "2021 and before", as.character(.data$year)
    ),
    group = factor(x = .data$group, levels = unique(.data$group))
  ) |>
  dplyr::group_by(.data$group) |>
  dplyr::group_split() |>
  as.list() |>
  purrr::map_chr(\(x) {
    title <- unique(x$group)
    articles <- paste0(x$cite, collapse = "\n")
    paste0("### ", title, "\n\n", articles, "\n")
  }) |>
  paste0(collapse = "\n")
```
